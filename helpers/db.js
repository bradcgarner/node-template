...
const formatTimestampForSql = (value, sqlOption='raw') => {
  ...
  const o = sqlOption === 'raw' ? 'raw' : 'knex' ;
...
const formatDataForSql = (data, key, option='raw') => {
  ...
  if(Array.isArray(data)) {
    ...
    if(key.includes('imestamp')){
      processedArray = data.map(timestamp=>formatTimestampForSql(timestamp, o));
    } 
    ...
    const processedArray2 = processedArray.map(item=>{
      ...
      if(o === 'raw'){
        ...
        return `"${item}"`; // arrays of strings should have all strings in double quotes
      }
      return item;
    });
    // stringify if raw
  ...
  if(key.includes('imestamp')){
    return formatTimestampForSql(data, o);
  }
  if(typeof data === 'string'){
    const noSingleQuote = data.replace('\'', '');
    if(o==='raw') {
      return `'${noSingleQuote}'`;
    } else {
      return noSingleQuote;
    }
  } // end string
...
const formatObjectForKnex = object => {
  ...
  for (let key in object) {
    o[key] = formatDataForSql (object[key], key, 'knex');
  }
  return o;
};
...
const createSqlFetchTableKeys = input => {
  ...
  logger.info('createSqlFetchTableKeys input', input);
  ...
  logger.info('joinTosArray', joinTosArray);
  logger.info('joinTos', joinTos);
  logger.info('keyLocsArray', keyLocsArray);
  logger.info('keyLocs', keyLocs);
  logger.info('joinTypesArray', joinTypesArray);
  logger.info('joinTypes', joinTypes);

  const joinStatementArray = tables.map((joinTo, i)=>{
    logger.info('i', i);
    const joinFrom = tables[joinTos[i]]; 
    const keyLoc   = tables[keyLocs[i]];
    logger.info('joinTo', joinTo, 'joinFrom', joinFrom, 'keyLoc', keyLoc);
